...
cpoPairWisePerGroup <- function(m, time, site, spp.group,
                                   occurrence.tresh, ...) {
  ...
  for(i in seq_along(site.lab) ) {
    ...
    for(j in seq_along(time.lab) ) {
      ...
      if( ncol(m.tmp) > 1 & nrow(m.tmp) > 1 ) {
        ...
        cpoPairwiseTmp <- cpoPairWise(m.tmp,
        ...
        if( extra.arg[['null.model']] ) {
          ...
          null.pairwise.mean.tmp <- c( cpoPairwiseTmp$null.dist.mean )
          null.pairwise.sd.tmp <- c( cpoPairwiseTmp$null.dist.sd )
          cpoPairwiseTmp <- c( cpoPairwiseTmp$CPO )

        } else {

          cpoPairwiseTmp <- c( cpoPairwiseTmp )

        }


        index.pos.tmp <- seq_along(cpoPairwiseTmp)

        valid.index.tmp <- !is.na(cpoPairwiseTmp)

        c.score.vals <- cpoPairwiseTmp[valid.index.tmp]
